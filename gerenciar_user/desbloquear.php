...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  $delete = $obj->desbloquearUsuarios($id_usuario);
  $obj_credenciais->updateDesbloquear($id_usuario);
  header("Location: index.php");
  exit();
}
?>
    ...
    <div class="authenticated-layout__content">
      <div class="dashboard-route">
        ...
        <div class="dashboard-route__first-group">
          <form style="width: 100%" class="card-panel" action="desbloquear.php?id=<?= $id_usuario?>" method="POST">
            <div
              ...
              <a style="cursor: pointer;" onclick="window.history.back()"><img src="/assets/left-arrow.svg"
                  alt=""></a>DESBLOQUEAR USUÁRIO
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px">
              <div style="margin-right: 10px;">
                <p>Deseja realmente desbloquear o usuário <?= $usuarios['nome_usuario'] ?>?</p>
                <br>
                <button type="submit" id="deletar" class="btn-bloquear">Desbloquear usuário</button>
          </form>
        </div>
      </div>
    </div>
